...
- [~] **C3** Attendance PWA — **foundation set:** private `attendance-selfies` storage bucket created; `selfie_url` is NOT NULL (so selfie is **required** by schema — answers Q3); `geo_fences` is fully configurable (lat/lng/radius/kind) so I'll make it admin-data (no coords needed from you). **Build next:** geofence-config admin, `/attendance` check-in (camera selfie + GPS + geofence check → `attendance_records`), manager view. ⚠️ The camera (`getUserMedia`) can't be verified headless — needs your phone to test.
