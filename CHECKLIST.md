...
- [x] **C2** People — `/team/[id]` employee detail + `/freelancers` roster+detail + **`/talents` roster** (category/niches/commission/contract) + **assignment composer** (project team form picks members **or freelancers** → `project_assignments` + `write_activity`). All linked from `/team`; **`/talents/[id]` detail** (contract/commission/niches + managed accounts) done. **Minor remaining:** freelancer availability table (new schema).
