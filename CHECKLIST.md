...
- [x] Phase D — **DONE + deployed + view-as VERIFIED 2026-05-29** (migration 051). 6 `v_*_safe` views (projects/clients/contacts/equipment/email_threads/**team**) per spec Part 2 + `user_assigned_to_project()`/`has_same_department()` helpers; Drizzle `safe_views.ts` (`.existing()`). **8 read pages genuinely switched** to views via `withProfileScope` (projects list+detail, crm, clients/[id], equipment list+detail, inbox, team/[id]) — verified by grep + build + **live view-as**: as خالد (project_manager) /projects shows only his 3 assigned projects with القيمة column masked to "—" (vs 12 projects + values as GM). Masking smoke 11/11.
