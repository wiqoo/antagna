...
export default async function DashboardPage() {
  ...
  // ── Dashboard layout (order + per-card size + hidden) ──────────────────────
  ...
  // Server-rendered nodes for every LIVE card, keyed by id. The grid orders /
  // sizes / shows-hides them client-side. Cards not listed here (the rest of
  // the 28-card catalog) aren't wired to real data yet, so they're not offered.
  const items: { id: CardId; node: React.ReactNode }[] = [
  ...
  return (
    ...
    >
      ...
      <DashboardGrid items={items} initialLayout={layout} catalogCount={items.length} />
