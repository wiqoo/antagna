...
قواعد:
- **ممنوع منعاً باتاً اختراع أي رقم أو اسم.** استخدم فقط الأرقام والأسماء الواردة حرفياً في قسم «أحمال اليوم». إن كان العدد صفراً أو غير مذكور، لا تذكر رقماً ولا تفترض وجود عناصر (مثال خاطئ: «وافِق على ٥٢٤ طلباً» بينما العدد صفر).
- اربط كل بند ببيانات النظام الفعلية: إن كانت الموافقات = ٣، اكتب «راجِع الموافقات الثلاث المعلّقة»؛ إن كانت صفراً، لا تذكر الموافقات إطلاقاً.
- بالعربية الفصحى، صيغة أمر، ملموسة. لا عموميات مثل «راجع البريد».
...
function buildRoutineContext(
  ...
  // ALWAYS print the real counts (including zeros) so the model is grounded and
  // never invents numbers like "524 pending approvals".
  lines.push('', 'أحمال اليوم الفعلية (أرقام حقيقية من النظام — اعتمد عليها وحدها):');
  lines.push(
    `- تصوير اليوم: ${signals.shoots.length}` +
      (signals.shoots.length
        ? ' — ' +
          signals.shoots
  ...
  lines.push(
    `- مهام مشاريع مستحقّة: ${signals.tasksDue.length}` +
      (signals.tasksDue.length
        ? ' — ' + signals.tasksDue.map((t) => `${t.title}${t.project ? ` [${t.project}]` : ''}`).join('، ')
        : ''),
  );
  lines.push(
    `- مهام يوميّة مستحقّة: ${signals.dailyDue.length}` +
      (signals.dailyDue.length ? ' — ' + signals.dailyDue.map((t) => t.title).join('، ') : ''),
  );
  lines.push(`- موافقات تنتظرك: ${signals.approvals}`);
  lines.push(
    `- محادثات تنتظر ردّك: ${signals.threads.length}` +
      (signals.threads.length
  ...
  if (totalSignals === 0) {
    lines.push(
      '',
      'كل الأرقام أعلاه صفر اليوم. اقترح بنوداً استباقية مبنية على مسؤولياته ومؤشراته فقط، دون ذكر أي أرقام أو عناصر غير موجودة.',
    );
  }
  lines.push('', 'أخرج خطة اليوم JSON فقط، معتمداً حصراً على الأرقام الحقيقية أعلاه.');
  return lines.join('\n');
}
...
/**
 * Clear today's AI routine so the next /dashboard render regenerates it fresh
 * (with the current data + prompt). Bound to the "جدّد الخطة" button — simpler
 * than threading signals through a form; the page re-gathers them on re-render.
 */
export async function clearTodayAiRoutine(): Promise<void> {
  const actorId = await requirePermissionAction('daily_task.manage_self');
  const prefix = `ai_routine:${riyadhToday()}:`;
