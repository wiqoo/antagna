    ...
    <Shell user={{ email: user.email ?? '' }} activePath="/crm">
      ...
      <Card className="relative overflow-hidden">
        ...
        <div className="relative flex flex-wrap items-start justify-between gap-6">
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-1.5">
              ...
              {client.nameEn && (
                <p className="text-sm text-[--text-muted]">{client.nameEn}</p>
              )}
          ...
          <Link
            ...
            className="inline-flex h-9 items-center gap-1.5 rounded-xl border border-[--line] bg-[--surface] px-3 text-sm hover:border-[--accent]"
          >
            <Pencil size={14} />
            تعديل
          </Link>
        ...
        {(client.vatNumber || client.crNumber || client.legalName) && (
          <div className="relative mt-6 grid grid-cols-1 gap-3 border-t border-[--line] pt-4 text-xs md:grid-cols-3">
            {client.legalName && (
              <div>
                <p className="text-[--text-dim]">الاسم القانوني</p>
                <p className="mt-0.5 text-[--text]">{client.legalName}</p>
              </div>
            )}
            {client.vatNumber && (
              <div>
                <p className="text-[--text-dim]">VAT</p>
                <p className="mt-0.5 font-mono text-[--text]">{client.vatNumber}</p>
              </div>
            )}
            {client.crNumber && (
              <div>
                <p className="text-[--text-dim]">CR</p>
                <p className="mt-0.5 font-mono text-[--text]">{client.crNumber}</p>
              </div>
            )}
          </div>
        )}
        ...
        <div className="p-6 pb-4">
          ...
          >
            ...
            <input
              ...
              name="email"
              placeholder="email"
              className="h-9 rounded-xl border border-[--line] bg-[--bg-elevated] px-3 text-sm font-mono"
            />
      ...
      {client.notes && (
        <Card>
          <h3 className="mb-2 text-xs font-semibold uppercase tracking-[0.15em] text-[--text-dim]">
            ملاحظات
          </h3>
