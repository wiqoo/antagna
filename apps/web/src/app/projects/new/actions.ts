...
export async function createProject(formData: FormData) {
  ...
  // ════════════════════════════════════════════════════════════════════════
  // DRIVE — create the project folder tree in the BACKGROUND (after()) so the
  // ~20 Google Drive API round-trips (6-12s) never block the redirect. The user
  // lands on the project page instantly; the folder + drive_folder_url fill in
  // a few seconds later (same Fluid Compute invocation, runs after the response
  // flushes). Best-effort: a Drive outage must NEVER surface — any error is
  // swallowed and the idempotent cron (POST /api/integrations/drive/scan)
